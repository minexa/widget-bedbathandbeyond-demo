/* SiteCatalyst code version: H.26.2.   {Revised 010/13/2015}}
...
if(hostName.contains("www.bedbathandbeyond.com")){
    var s_account="bbbprod";//prod
}
else{
	var s_account="bbbdev"  //development 
}
...
s.linkInternalFilters="javascript:,www.bedbathandbeyond.com,m.bedbathandbeyond.com,www2.bedbathandbeyond.com,an.ows2.bedbathandbeyond.com,sd.ows2.bedbathandbeyond.com"
+"ist.bedbath.com,bbbqa.bedbath.com,bbbuat.bedbath.com,video.bedbathandbeyond.com,bbbdevatg.bedbath.com,m.devatg.bbby.biz,.itestbbby.com,itesttbsbbby.com,.itestTbsbbby.com"
...
/*scene7 video tracking code*/ 
s.internalCounter = 0;

function s7track(eventInfo, rolloverInfo) {
   var eventValues = eventInfo.split(',');
   var eventType = eventValues[0].toString();
/* MAP THE EVENTS TO THE CORRECT EVENTS */
   if (eventType == "HREF") {
	s.linkTrackVars='eVar1';
	s.linkTrackEvents='None';
	if(s.pageName.indexOf('Content Page>')>-1)
		s.eVar1="Circular>"+s.pageName.split('Content Page>')[1];
	else if(s.pageName.indexOf('Circular>')>-1)
		s.eVar1=s.pageName;
	else
		s.eVar1='circular';  //just in case page name is not expected value
	productMerch();
	return s.tl();
   }
...
/*read cookie and set country and currency code*/
var countryCurrency=s.split(getCookie('BedBathUS1ntsh1'),":");
var country=countryCurrency[0];
var currency=countryCurrency[1];
if(country)
	s.eVar52=s.prop26=country;
else
	s.eVar52=s.prop26="US";	
if(currency)
...
/* set eVar21 equal to app for traffic directed from app to mobile web site via webview */	
var expCookieSet="";
expCookieSet=s.c_r('exp_type');    //read cookie to see if it has been set
if (expCookieSet==""){
	s.eVar21=s.getQueryParam('type');    //cookie not set, look for type parameter
	s.c_w('exp_type',s.eVar21,0);        //set cookie with eVar21 value 
}
else s.eVar21=s.c_r('exp_type');  

/*set prop6 and eVar9 equal to the domain of site*/
s.prop6=s.eVar9=document.domain;
/*set eVar19,eVar20,eVar21 based on domain value */
if (!s.eVar20){
	if (document.domain.indexOf('.com')>-1)
		s.eVar20='US';
	else if (document.domain.indexOf('.ca')>-1)
		s.eVar20='CA';
}
if(!s.eVar19){	
	if (document.domain.indexOf('buybuybaby')>-1||document.domain.indexOf('baby')>-1)
		s.eVar19= 'Buy Buy Baby';
	else if (document.domain.indexOf('bedbathandbeyond')>-1||document.domain.indexOf('bbb')>-1||document.domain.indexOf('ist.bedbath')>-1)
		s.eVar19= 'Bed Bath & Beyond';	
}
if (!s.eVar21){
	if (document.domain.indexOf('digby')>-1||document.domain.indexOf('m.')>-1)
		s.eVar21= 'mobile';
	else
		s.eVar21= 'web';	
}
s.linkTrackVars=s.apl(s.linkTrackVars,'currencyCode,eVar52,prop26,prop6,eVar9,eVar19,eVar20,eVar21,prop16,eVar16,eVar38,prop23',',',2);

//remove spaces in page name values on desktop
if(s.pageName&&s.pageName.indexOf(' > ')>-1)
	s.pageName = s.repl(s.pageName,' > ','>');	

//remove leading m: in page name values on mobile
if(s.pageName&&s.pageName.indexOf('m:')>-1)
	s.pageName = s.repl(s.pageName,'m:','');	
	
//fix channel, prop2, prop3, and eVar4 values - replace &amp; with &
if(s.channel&&s.channel.indexOf('&amp;')>-1)
	s.channel = s.repl(s.channel,'&amp;','&');	
if(s.prop2&&s.prop2.indexOf('&amp;')>-1)
	s.prop2 = s.repl(s.prop2,'&amp;','&');	
if(s.prop3&&s.prop3.indexOf('&amp;')>-1)
	s.prop3 = s.repl(s.prop3,'&amp;','&');
if(s.prop20&&s.prop20.indexOf('&amp;')>-1)
	s.prop20 = s.repl(s.prop20,'&amp;','&');	
if(s.eVar4&&s.eVar4.indexOf('&amp;')>-1)
	s.eVar4 = s.repl(s.eVar4,'&amp;','&');	

//fix mobile values for eVar16
	if (s.eVar16&&s.eVar16=="Anonymous")
		s.eVar16="Non registered user";
	if(s.eVar16&&s.eVar16=="Registered")
		s.eVar16="registered user";
		
/*set prop16 equal to the value set in eVar16 for user type*/
if (s.eVar16)
	s.prop16=s.eVar16;
...
/*get previous pageName value  */
s.prop10=s.getPreviousValue(s.pageName,'gpv','');

/*set prop11 equal to the percentage of the previous page viewed */
if (s.prop10)
	s.prop11=s.getPercentPageViewed();


/*When the events variable contains prodView (and no event57 or event54), append event6*/
if(s.events&&s.events.indexOf('prodView')>-1&&!(s.events.indexOf('event57')>-1)&&!(s.events.indexOf('event54')>-1))
	s.events=s.apl(s.events,'event6',',',2);

/*When the events variable contains event57 (quickview) or event54 (collection view) and no prodview, append prodView but NOT event6*/
if(s.events&&s.events.indexOf('event57')>-1&&!(s.events.indexOf('prodView')>-1))
	s.events=s.apl(s.events,'prodView',',',2);
if(s.events&&s.events.indexOf('event54')>-1&&!(s.events.indexOf('prodView')>-1))
	s.events=s.apl(s.events,'prodView',',',2);
	
/*set eVar11 equal to purchaseID  */	
if(s.purchaseID)
	s.eVar11=s.purchaseID;

/*Set eVar2 equal to prop7. Add event1 to the list of events when prop7 has a value.  
When no results are returned, set prop8 equal to "zero" and add event2 to the list of events.  */
if(s.prop7)
{
	s.eVar2=s.prop7;
...
/* Automate Campaign Tracking Code Extraction based on the mcid parameter*/
if(!s.campaign)
	var extCampaign=s.getQueryParam('mcid');
var checkVal=s.getValOnce(extCampaign,"s_campaign",0);
if(!checkVal=="")
	s.campaign=extCampaign;

/*set eVar3 equal to icid parameter  */
...
/*Shop by brand fix  */
var brandpage=s.getQueryParam('frmBrandPage');
if (brandpage=='true')
{
	s.eVar4="Shop By Brand";
	s.eVar5=s.eVar4 +">" + s.prop7;
}

/*condition to clear search event and props on internal campaign and browse pages that redirect to search results pages*/
/*Also adding external campaigns that have a search result landing page*/
if ((s.eVar3||s.eVar4||s.campaign)&&s.events.indexOf('event1')>-1)
{
	var a=s.split(s.events,',');
		var e='';
		for(var i = 0; i < a.length ; i++ )
...
/*Set Account Detail when wish list is manipulated  */
if(s.events&&((s.events.indexOf('event37')>-1)||(s.events.indexOf('event35')>-1)||(s.events.indexOf('event34')>-1)))
	s.eVar26="wish list";
if(s.events&&s.events.indexOf('event30')>-1)
	s.eVar26="account overview"



if(s.campaign){ 
	if(s.eo===undefined){  //page load only, so external campaign does not overwrite cross-sell PFMs
...
else if(!s.eVar1&&s.eVar3&&s.eVar3!='non-internal campaign'){ 
...
else if(!s.eVar1&&s.eVar4&&s.eVar4!='non-browse'){ 
	s.eVar1='browse';
	s.eVar2='non-search';
	s.eVar3='non-internal campaign';
	s.eVar7='non-cross sell';
...
else if(!s.eVar1&&s.eVar2&&s.eVar2!='non-search'){
...
else if(!s.eVar1&&s.eVar24&&s.eVar24!='non-registry'&&s.prop1!='Product Details Page'){	
	s.eVar1='registry';
	s.eVar2='non-search';
	s.eVar3='non-internal campaign';
	s.eVar4='non-browse';
...
else if (!s.eVar1&&s.eVar26&&s.eVar26=='wish list'&&s.prop1!='Product Details Page'){
	s.eVar1='wish list';    
	s.eVar2='non-search';
	s.eVar3='non-internal campaign';
	s.eVar4='non-browse';
...
{	
	...
	s.eVar2=s.eVar3=s.eVar4=s.eVar5=s.eVar6=s.eVar7=s.eVar8='D=v1';
}
else if(s.eVar1)  
{
	s.eVar2='non-search';
	...
	s.eVar5='D=v4';
	s.eVar6='D=v4';
	if (!s.eVar7){  //the cross-sell function sets eVar1,eVar7, and eVar8.  This condition will set eVar7 and eVar8 if eVar7 does not contain a value.
		s.eVar7='non-cross sell';
		s.eVar8='D=v7';
	}
}
/* create productmerch product for merchandising eVar binding - not seting if cross-sell because set in cross-sell function */
	if(s.eVar1 && (!s.products||(s.products&&s.products.indexOf(';productmerch')>-1)) && (s.p_fo('onemerch')==1||(s.linkType!=''&&s.linkTrackVars.indexOf('eVar1')>-1)) && !s.events.indexOf('event3')>-1)
	{
		productMerch();
	}
...
/*set prop18 equal to a combination of the current day and current hour.   
Set eVar22 equal to the value of prop8 via dynamic syntax. */
s.prop18=s.getTimeParting('h','-5') + "|" + s.getTimeParting('d','-5') + "|" + s.getTimeParting('w','-5');
s.eVar22="D=c18";
...
s.socialPlatforms('eVar43');

/*capture page URL */
if(window.location.href=="http://www.bedbathandbeyond.com/"||window.location.href=="https://m.bedbathandbeyond.com/m/")
	s.prop28="hp";
else if(window.location.href.split('?')[1])
	s.prop28=window.location.pathname+"?"+window.location.href.split('?')[1];
else
	s.prop28=window.location.pathname;
...
function s_crossSell(desc)
{ 
  s.eVar1=desc;
  s.linkTrackVars='None';
  s.linkTrackVars='events,eVar7,eVar8,eVar1,eVar2,eVar3,eVar4,eVar5,eVar6';
  s.linkTrackEvents='event3';
  s.events='event3';
  s.eVar2='non-search';
  s.eVar3='non-internal campaign';
  s.eVar4=s.eVar5=s.evar6='non-browse';
  s.eVar7=s.pageName;
  if(s.products&&!(s.products.indexOf(';productmerch')>-1)){
    var prodId=s.products.substring(1).split(";");
	s.eVar8=prodId[0];
	}
  else {
...
/*Convenience function to track registry creation process  */
function createRegistry(s_registryStep){
    s.linkTrackVars='events';
    var tempEvent=""; 
    if(s_registryStep=='1'){
	tempEvent="event25";	
    }
    else if (s_registryStep=='2'){
	tempEvent="event26";
    }
    else if (s_registryStep=='3'){
 	tempEvent="event27";
    }
    else{
    //default the event to Step 1
	tempEvent="event25";
	var s_registryStep='1 (default)';
    }	
    s.linkTrackEvents=tempEvent;
    s.events=tempEvent;
    s.tl(true,'o','create registry - step' + s_registryStep);
	s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture refinement tracking  */
function refinementTrack(refine_type,refine_val){
s.linkTrackVars='prop13,prop14,prop15,eVar39';
s.prop13=s.pageName;
s.prop14=refine_type;
s.prop15=refine_val;
s.eVar39=refine_type + ':' + refine_val;
s.tl(true,'o', 'refinements');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture email signup  */
...
/*Convenience function to capture account updates  */
function acctUpdate(updateDetail){
s.linkTrackVars='events,eVar26';
s.linkTrackEvents='event31';
s.events='event31';
s.eVar26=updateDetail;
s.tl(true,'o', 'my account updates');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture 3rd party links/components updates  */
function externalLinks(linkVal){
s.linkTrackVars='prop13,prop12';
s.prop13=s.pageName;
s.prop12=linkVal;
s.tl(true,'o', 'third party link');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture internal links/components updates  */
function internalLinks(linkVal){
s.linkTrackVars='prop13,prop12';
s.prop13=s.pageName;
s.prop12=linkVal;
s.tl(true,'o', 'internal link');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture Find in Store  */
function findInStore(prodId){
s.linkTrackVars='events,products';
s.linkTrackEvents='event41';
s.events='event41';
s.products=';'+prodId;
s.tl(true,'o', 'find in store button');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture product detail page elements  */
function prodDetailElement(prodId,element){
s.linkTrackVars='eVar25,eVar51,products,events';
s.linkTrackEvents='event47';
s.events='event47';
s.products=';' + prodId + ';;;;eVar25=' + element;
s.eVar51=element;
s.tl(true,'o', 'product detail page element interaction');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture college searches  */
function collegeSearch(college){
s.linkTrackVars='eVar27,events';
s.linkTrackEvents='event40';
s.events='event40';
s.eVar27=college;
s.tl(true,'o', 'college search');
s.manageVars("clearVars","s.linkTrackVars",1);
}
/*Convenience function to capture Custom Link  */
function customLinkTracking(desc){
s.tl(true,'o',desc);
s.manageVars("clearVars","s.linkTrackVars",1);
}


/*Convenience function to capture email registry*/
function emailRegistry(){
s.linkTrackVars='events,eVar23,eVar24';
s.linkTrackEvents='event51';
s.events='event51';
s.tl(true,'o', 'email registry');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture search tab*/
function searchTabTrack(tab_val,tab_resultsnum){
s.linkTrackVars='prop13,prop9,prop8,prop7,eVar41';
s.prop13=s.pageName;
s.prop9=tab_val;
s.eVar41=tab_val;
s.tl(true,'o', 'search tabs');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture sort value*/
function sortTrack(sort_val){
s.linkTrackVars='prop13,prop24,eVar40';
s.prop13=s.pageName;
s.prop24=sort_val;
s.eVar40=sort_val;
s.tl(true,'o', 'sort');
...
/*Mobile Specific Convenience function to capture when a new cart is opened and a product is added  */
function addToNewCart(prodId,sku,location){
s.linkTrackVars='products,eVar30,eVar15,events';
s.linkTrackEvents='scAdd,scOpen';
s.events='scAdd,scOpen';
var checkLocation=getQueryVariable('showReturnRegistry');
if (checkLocation=="return")
		s.products=';'+ prodId +';;;;eVar30='+ sku +'|eVar15=registry';
else	
s.products=';'+ prodId +';;;;eVar30='+ sku +'|eVar15=' + location;
s.tl(true,'o','open cart and add product');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Mobile Specific Convenience function to capture when a product is added  */
function addToCart(prodId,sku,location){
s.linkTrackVars='products,eVar30,eVar15,events,eVar23,eVar24';
...
if (checkLocation=="return")
	...
	s.products=';'+ prodId +';;;;eVar30='+ sku +'|eVar15=' + location;
s.tl(true,'o','add product to cart');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Mobile Specific Convenience function to capture when an item is removed from cart   */
function removeFromCart(prodId,sku){
s.linkTrackVars='products,eVar30,events';
s.linkTrackEvents='scRemove';
s.events='scRemove';
s.products=';'+ prodId +';;;;eVar30='+ sku;
s.tl(true,'o','remove product from cart');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Mobile Specific Convenience function to capture when an item is removed from registry   */
function removeFromRegistry(prodId,sku,registryType,registryId){
s.linkTrackVars='products,eVar30,eVar23,eVar24,events';
s.linkTrackEvents='event19';
s.events='event19';
s.products=';'+ prodId +';;;;eVar30='+ sku;
s.eVar23=registryType;
s.eVar24=registryId;
s.tl(true,'o','remove product from registry');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Mobile Specific Convenience function to capture when an item is added to registry      */
function addToRegistry(prodId,sku,qty,rev,registryType,registryId) {
s.linkTrackVars='products,eVar30,eVar23,eVar24,events';
s.linkTrackEvents='event18,event22,event23';
s.events='event18,event22,event23';
s.products=';'+ prodId +';;;event22='+ qty +'|event23='+ rev +';eVar30='+ sku;
s.eVar23=registryType;
s.eVar24=registryId;
s.tl(true,'o','add product to registry');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture remove from wish list  */
function removeWishList(prodId,sku){
s.linkTrackVars='products,eVar30,events';
s.linkTrackEvents='event34';
s.events='event34';
s.products=';'+ prodId +';;;;eVar30='+ sku;
s.tl(true,'o', 'remove from wish list');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture update to wish list  */
function updateWishList(prodId,sku){
s.linkTrackVars='products,eVar30,events';
s.linkTrackEvents='event35';
s.events='event35';
s.products=';'+ prodId +';;;;eVar30='+ sku;
s.tl(true,'o', 'update wish list');
s.manageVars("clearVars","s.linkTrackVars",1);
}
...
/*Convenience function to capture coupon wallet creation*/
function couponWalletCreate(){
s.linkTrackVars='events';
s.linkTrackEvents='event69';
s.events='event69';
s.tl(true,'o', 'coupon wallet:create');
s.manageVars("clearVars","s.linkTrackVars",1);
}
/*Convenience function to capture coupon wallet login*/
function couponWalletLogin(){
s.linkTrackVars='events';
s.linkTrackEvents='event71';
s.events='event71';
s.tl(true,'o', 'coupon wallet:login');
s.manageVars("clearVars","s.linkTrackVars",1);
}
/*Convenience function to capture coupon wallet add*/
function couponWalletAdd(couponName){
s.linkTrackVars='events,prop27';
s.linkTrackEvents='event70';
s.events='event70';
s.prop27=couponName;
s.tl(true,'o', 'coupon wallet:coupon add');
s.manageVars("clearVars","s.linkTrackVars",1);
}
/*Convenience function to capture Coupon Wallet Action  */
function couponWalletAction(couponName,action){
s.linkTrackVars='prop13,prop12,prop27';
s.prop13=s.pageName;
s.prop12=action;
s.prop27=couponName;
s.tl(true,'o','coupon wallet:'+ action);
s.manageVars("clearVars","s.linkTrackVars",1);
}

function categoryLinkClick(container,link){
s.linkTrackVars="events,eVar50";
s.linkTrackEvents="None";
s.eVar50=s.pageName+">"+container+">"+link;
productMerch();
s.tl(this,'o','category link click');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience functin to capture registry tabs */
function registryTabTrack(tab_val){
s.linkTrackVars='prop29,eVar23,eVar24,eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8';
s.prop29=tab_val;
if(tab_val=="My Items"||tab_val=="my items")
	s.eVar1="registry";
else
	s.eVar1="Registry "+tab_val;
s.eVar2='non-search';
s.eVar3='non-internal campaign';
s.eVar4='non-browse';
s.eVar5='D=v4';
...
s.eVar8='D=v7';
productMerch();
s.tl(true,'o', 'registry tabs');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture UGC click*/
function ugcImageClick(){
s.linkTrackVars='eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8';
if(s.pageName){
		if(s.pageName.indexOf('UGC Listing')>-1)
			s.eVar1='UGC Gallery';
		else 
			s.eVar1='UGC';
s.eVar2='non-search';
...
s.eVar8='D=v7';
productMerch();
s.tl(true,'o','UGC click-'+s.pageName);
}
else{
	s.eVar1='UGC';
	s.eVar2='non-search';
	...
	s.eVar8='D=v7';
	productMerch();
	s.tl(true,'o','UGC click-unknown page');
} 
s.manageVars("clearVars","s.linkTrackVars",1);
}


/*Convenience function to capture social share */
function socialShare(prodId,shareType){
s.linkTrackVars='products,prop13,prop30,events';
...
if(s.pageName)
	s.prop13=s.pageName;
s.tl(true,'o','social share:'+shareType);
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture exit to social site (BBB FB, Twitter, Instagram page */
function socialExit(desc){
s.linkTrackVars='prop12,prop13';
s.prop13=s.pageName;
s.prop12='social exit:'+desc;
s.tl(true,'e',s.eo);
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture registry upload for UGC image*/
function ugcRegistryUpload(uploadType){
s.linkTrackVars='eVar23,eVar24,prop31';	
s.prop31=uploadType;
s.tl(true,'o','UGC-registry image upload');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture registry click of UGC image*/
function ugcRegistryImageClick(){
s.linkTrackVars='eVar23,eVar24,events';	
s.linkTrackEvents='event80';
s.events='event80';
s.tl(true,'o','UGC-registry thumbnail click');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture buy off registry click*/
function buyOutsideRegistry(prodId,sku,regId,regType){
s.linkTrackVars='products,eVar30,eVar23,eVar24,eVar55';
s.linkTrackEvents='None';
s.products=';'+ prodId +';;;;eVar30='+sku+'|eVar55=buy outside registry';
s.eVar23=regType;
s.eVar24=regId;
s.tl(true,'o','buy outside registry click');
s.manageVars("clearVars","s.linkTrackVars",1);
}

/*Convenience function to capture personalize this impression on PDP, QuickView*/
function pdpPersonalize(prodId,sku){
s.linkTrackVars='products,eVar30,events';
s.linkTrackEvents='event81';
s.events='event81';
s.products=';'+ prodId +';;;;eVar30='+sku;
s.tl(true,'o','Personalize This: presented');
s.manageVars("clearVars","s.linkTrackVars",1);
}
/************************** PLUGINS SECTION *************************/
...
/*
 * Plugin: getPercentPageViewed 1.55 (updated to resolve issue in app)
 */
...
+"),a=(v&&v.indexOf(',')>-1)?v.split(',',4):[];if(a&&a.length<4){for(var"
...
+"'SocialAuthor');s.Integrate.SocialAuthor.tEvar='eVar73';s.Integrate"
...
/* Media Module Config */
/*Client only wants to capture if first 20 sec of video was viewed */
/*(20% is roughly first 20 secs of videos with avg length of 100 secs )           */
s.loadModule('Media');
s.Media.autoTrack=false;
s.Media.trackWhilePlaying=false;
s.Media.playerName='liveclicker';
s.Media.segmentByMilestones=true;
s.Media.trackMilestones="20";
s.Media.trackUsingContextData=true;
s.Media.contextDataMapping = {
"a.media.name":"eVar32,prop19",              
      "a.media.segment":"eVar33",
	  "a.contentType":"eVar34",
      "a.media.timePlayed":"event44",
...
s.Media.trackVars='events,eVar32,eVar33,eVar34,prop19';
s.Media.trackEvents='event48,event46,event45,event44';
...
s.visitorNamespace="bedbathbeyond"
//s.trackingServer="bedbathbeyond.d1.sc.omtrdc.net"
s.trackingServer="metrics.bedbathandbeyond.com"
s.trackingServerSecure="smetrics.bedbathandbeyond.com"
